Shader "Lpk/LightModel/ToonLightBase"
{
    Properties
    {
        ...
        [Space]   
        _OutlineWidth      ("OutlineWidth", Range(0.0, 10.0))      = 2.5
            ...
            float4 frag(Varyings input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
            ...
            float2 uv = input.uv;
                ...
                // --- NEW COLOR LOGIC ---
                // if _SecondColor is not white, use it instead of _BaseColor
                float isWhite = all(_SecondColor.rgb >= 0.999); // tolerance
                float3 activeColor = lerp(_SecondColor.rgb, _BaseColor.rgb, isWhite);

                // Specular, rim, and lighting as before
                ...
                float3 diffuse = _MainLightColor.rgb * baseMap.rgb * activeColor * shadowNL * shadow;
                float3 specular = _SpecularColor.rgb * shadow * shadowNL * specularNH;
                float3 ambient = rim * _RimColor.rgb + SampleSH(N) * activeColor * baseMap.rgb;
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                // Push along normal in world space (not affected by object scaling)
                positionWS += normalWS * _OutlineWidth * 0.25; // SCALING FACTOR!!!!!!!!!!!
